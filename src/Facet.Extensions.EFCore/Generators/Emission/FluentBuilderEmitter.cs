...
internal static class FluentBuilderEmitter
{
    public static void Emit(SourceProductionContext context, ModelRoot efModel, ImmutableArray<FacetDtoInfo> facetDtos)
    {
        ...
        // Generate the main DbContext extension method
        GenerateDbContextExtensions(context, efModel, facetDtos);
    }
    ...
    private static void GenerateFluentBuilder(SourceProductionContext context, EntityModel entity, FacetDtoInfo dtoInfo, ContextModel contextModel)
    {
        ...
        // Constructor
        sb.AppendLine($"    internal {builderName}(IQueryable<{entity.Clr}> query)");
        sb.AppendLine("    {");
    ...
    private static void GenerateDbContextExtensions(SourceProductionContext context, ModelRoot efModel, ImmutableArray<FacetDtoInfo> facetDtos)
    {
        ...
        var targetNamespace = firstDto?.DtoNamespace ?? "Facet.Extensions.EFCore";
        ...
        foreach (var contextModel in efModel.Contexts)
        {
            foreach (var entity in contextModel.Entities)
            {
                ...
                sb.AppendLine($"    /// </summary>");
                sb.AppendLine($"    public static {builderName}<I{entityName}Shape> Facet<TEntity, TDto>(this DbContext context)");
                sb.AppendLine($"        where TEntity : class");
                sb.AppendLine($"        where TDto : class");
                sb.AppendLine("    {");
                sb.AppendLine($"        var query = context.Set<TEntity>();");
                sb.AppendLine($"        return new {builderName}<I{entityName}Shape>(query as IQueryable<{entity.Clr}> ?? throw new InvalidOperationException(\"Invalid entity type\"));");
                sb.AppendLine("    }");
                sb.AppendLine();
            }
        }
