...
public sealed class FacetEfGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(combined, static (context, data) =>
        {
            ...
            // Report diagnostics for debugging
            if (efModel == null)
            {
            ...
            if (facetDtos.Length == 0)
            {
                ...
                // For debugging, still try to generate basic output if we have an EF model
                // This helps identify if the issue is with DTO discovery vs EF model loading
                if (efModel.Contexts.Any(c => c.Entities.Count > 0))
                {
                    // Generate a basic diagnostic file to confirm the generator is running
                    var diagnosticContent = $@"// <auto-generated />
// Diagnostic: EF Model found with {efModel.Contexts.Sum(c => c.Entities.Count)} entities, but no DTOs discovered
// Entities: {string.Join(", ", efModel.Contexts.SelectMany(c => c.Entities).Select(e => e.Name))}
";
            ...
            if (config.EnableDebugOutput)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    ...
                    Location.None,
                    efModel.Contexts.Count,
                    "EF Contexts",
                    string.Join(", ", efModel.Contexts.Select(c => c.Context))));
