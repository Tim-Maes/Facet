        ...
        while (current != null)
        {
            foreach (var member in current.GetMembers())
            {
                    ...
                    !visited.Contains(member.Name))
                {
                    ...
                    {
                        // Skip partial implementing declarations (C# 13+ partial properties).
                        // We want to process only the defining declaration, which carries the
                        // user-authored attributes (e.g., [ObservableProperty]). The implementing
                        // declaration is generated by another source generator and has accessor bodies.
