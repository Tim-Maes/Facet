...
public static class SelectorsEmitter
{
    ...
    private static void GenerateSelectors(SourceProductionContext context, EntityModel entity, FacetDtoInfo dtoInfo, ImmutableHashSet<string> usedChains)
    {
        ...
        // This ensures selectors can find the shape interfaces regardless of namespace differences
        ...
        // Shape implementation classes are now generated by ShapeInterfacesEmitter in consolidated file
        // Generate selectors class only
    ...
    private static void GenerateNavigationShapeSelector(StringBuilder sb, EntityModel entity, NavigationModel navigation, FacetDtoInfo dtoInfo)
    {
        ...
        // Use inline projection to avoid circular dependencies on concrete shape classes
        if (navigation.IsCollection)
        {
            ...
            sb.AppendLine("        {");
            GenerateScalarPropertyMappings(sb, entity, dtoInfo);
            sb.AppendLine($"            {navName} = entity.{navName}.Select(n => new {targetEntityName}Shape {{ Id = n.Id }}).ToList()");
            sb.AppendLine("        };");
        }
        else
        {
            ...
            sb.AppendLine("        {");
            GenerateScalarPropertyMappings(sb, entity, dtoInfo);
            sb.AppendLine($"            {navName} = entity.{navName} == null ? null! : new {targetEntityName}Shape {{ Id = entity.{navName}.Id }}");
            sb.AppendLine("        };");
        }
